...
- **Em uma vista de união, a galáxia de origem é sinalizada em vários lugares.** Quando há mais de uma galáxia em vista, o nome da galáxia do buraco de minhoca aparece como prefixo antes do nome do buraco de minhoca no modal de mídia 3D, na dica de sobrevoo e no mapa 2D, para que dois buracos de minhoca de mesmo nome, de galáxias diferentes, permaneçam distinguíveis à primeira vista. O tema do ícone do buraco de minhoca é mais uma pista rápida, e o cartão de informações nomeia a galáxia de origem no seu rodapé.
...
- **Sua operadora pode afrouxar o casamento de palavras-chave.** Por padrão, dois buracos de minhoca só se conectam quando carregam a *mesma* palavra-chave. Uma operadora pode ligar o casamento aproximado para a instância ou para um aglomerado, de modo que grafias próximas (um plural, um pequeno erro de digitação) também desenhem uma linha de relação entre galáxias. Se você vê conexões entre palavras que são próximas mas não idênticas, é por causa dessa configuração; ela é controlada no lado da administração, não por buraco de minhoca.
