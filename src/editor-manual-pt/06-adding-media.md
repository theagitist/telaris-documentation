...
![Aba Imagem no modal Adicionar buraco de minhoca: campo de URL e seletor de arquivo, além do interruptor Usar como ícone do buraco de minhoca](assets/images/editor-manual-pt/05-new-wormhole-modal.png)

Duas formas de anexar uma imagem:

- **URL ou arquivo de imagem**: cole o endereço de uma imagem hospedada em outro lugar na web. Use quando a casa da imagem é algo estável (um arquivo de museu, o site de uma comunidade, um CDN sob seu controle). A URL precisa apontar para a imagem em si, não para uma página que contém a imagem.
- **Seletor de arquivo**: escolha **Escolher arquivo** e envie uma imagem do seu computador. Telaris mantém o arquivo enviado na sua instância e o serve de volta para as visitantes.

De qualquer forma, a imagem vira o visual primário da visitante quando ela abre o buraco de minhoca.

**Usar como ícone do buraco de minhoca** é um interruptor abaixo dos campos de imagem. Quando ligado, a imagem substitui o ícone padrão do nó 3D do tema: as visitantes veem a imagem *como* o buraco de minhoca na cena 3D, e não só dentro do cartão de informações. Útil quando a imagem é a identidade do buraco de minhoca (um retrato, uma capa, um logotipo). Quando desligado, a cena 3D mostra o ícone padrão do tema e a imagem só aparece dentro do cartão aberto.

**Atribuição da imagem** é um pequeno campo de texto livre que viaja com a imagem. Use para crédito da fotógrafa, arquivo de origem, nota de licença, qualquer coisa que seria anexada à imagem se ela aparecesse em um livro impresso. As visitantes veem ao lado da imagem no cartão de informações.

## A aba Vídeo (MP4)

Use para trechos de filme, entrevistas gravadas, animações, qualquer coisa em movimento. Telaris suporta apenas MP4 neste momento; outros formatos (WebM, MOV, AVI) precisam ser transcodificados antes ou hospedados em um serviço de vídeo e embutidos (veja Código de incorporação abaixo).

![Aba Vídeo (MP4) no modal Adicionar buraco de minhoca: URL de vídeo / seletor de arquivo para um MP4](assets/images/editor-manual-pt/07-media-video-tab.png)
...
Se seu vídeo vive em um serviço de streaming (Vimeo, YouTube, archive.org), o campo Código de incorporação abaixo é a ferramenta certa, não a aba Vídeo (MP4).

## A aba PDF

Para documentos: artigos, guias de campo, fotocópias, relatórios, partituras, manuscritos.

![Aba PDF no modal Adicionar buraco de minhoca: URL de PDF / seletor de arquivo](assets/images/editor-manual-pt/08-media-pdf-tab.png)
...
O campo de áudio aparece abaixo das abas de visual primário. É independente da escolha de imagem / vídeo / PDF; um buraco de minhoca pode ter qualquer combinação de áudio e um visual primário.

- **URL ou arquivo de áudio**: cole uma URL para um arquivo de áudio ou faça upload de um. MP3 é a escolha mais segura; muitos navegadores também suportam OGG e WAV.

Quando o áudio está anexado, dois interruptores controlam a reprodução:

- **Reproduzir automaticamente**: quando ligado, o áudio começa assim que a visitante abre o cartão de informações. Quando desligado, a visitante vê um botão de play e inicia o áudio. A reprodução automática costuma ser certa para trilhas curtas, ambientes, ou atmosféricas; desligada é certa para fala, música, qualquer coisa que pede atenção.
- **Em loop**: quando ligado, o áudio reinicia do começo cada vez que termina. Use para loops ambientais, drones, paisagens sonoras; desligue para qualquer áudio com arco narrativo.
...
Encontre o campo **Código de incorporação** (abaixo das abas de visual primário) e cole o trecho exatamente como o serviço o entrega. Telaris não transforma nem higieniza código de embed; o que você cola é o que as visitantes recebem. Teste o buraco de minhoca depois abrindo-o em modo visitante; se o embed estiver quebrado, o campo é o lugar para arrumar.

O código de incorporação e o visual primário não são mutuamente exclusivos (o embed aparece separadamente no cartão de informações), mas para clareza costuma fazer sentido escolher um ou outro.

## O ícone personalizado

Abaixo dos campos de mídia, aparece um campo **URL ou arquivo de ícone**. Ele define a aparência do buraco de minhoca na cena 3D, *separadamente* do visual primário. Use quando:
...
- O interruptor Usar como ícone do buraco de minhoca na aba Imagem não basta porque você tem uma imagem *e* quer que o ícone da cena seja outra coisa.
...
- **A disposição livre é o modo de mídia mais expressivo e o menos tolerante.** O posicionamento absoluto aparece exatamente como você o fixou, o que também significa que ele não se reajusta para telas pequenas nem para tecnologia assistiva como o texto comum se reajusta. Use-a onde a composição importa; apoie-se nas abas clássicas Imagem ou PDF para documentos simples.
