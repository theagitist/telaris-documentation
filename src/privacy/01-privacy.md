...
- **Editor accounts.** Each instance keeps a list of editors authorised to add content. For each editor it stores a name, an email address, optional pronouns, and login timestamps, for as long as the account is active. An account may also hold a password hash, but a password is optional: an editor can instead sign in through a one-time link emailed to them each time, in which case no password is stored. See *Editors: your information and what you can ask for* below.
- **Self-service signup (only if the operator turns it on).** Some operators open their instance to self-service editor signup. When that is enabled, anyone can request an editor account by giving a name and an email address; the email is confirmed through a one-time link before the account can be used. A request that is never confirmed is removed automatically after a short period. Self-service signup is off unless an operator chooses to enable it; on most instances editor accounts are still created by the operator.
...
If you have an editor account on an instance, whether an operator created it for you or you signed up yourself, the following applies to you.

- **Why we keep your data: because you agreed.** Before you can use the editor, you are asked to review these documents and agree: at your first sign-in if an operator created your account, or at signup if you joined through self-service. Your agreement is recorded together with the version of each document, so that a later revision asks you again. You may withdraw your consent at any time.
- **What is stored about you:** your name, email address, any pronouns you provide, login timestamps, the editorial content you publish, and an audit record of administrative actions you take (in which your email is stored only as a salted hash, never in the clear). If you use a password, its hash is stored; if you sign in through emailed links instead, no password is stored. Short-lived, single-use sign-in tokens (for the email links that log you in, confirm a signup, or let you set a password) are stored only as hashes, and only until they are used or expire.
