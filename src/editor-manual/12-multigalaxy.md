...
- **In a union view, the source galaxy is signalled in several places.** When more than one galaxy is in view, the wormhole's galaxy name appears as a prefix before the wormhole name in the 3D media modal, the hover tooltip, and the 2D map, so two same-named wormholes from different galaxies stay distinguishable at a glance. The wormhole's icon theme is a further at-a-glance cue, and the info card names the source galaxy in its footer.
...
- **Your operator can loosen keyword matching.** By default two wormholes connect only when they carry the *same* keyword. An operator can turn on fuzzy matching for the instance or a cluster, so near-spellings (a plural, a small typo) also draw a relationship line across galaxies. If you see connections between words that are close but not identical, this setting is why; it is controlled on the admin side, not per wormhole.
